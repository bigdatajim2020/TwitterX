...
//  TwitterClone
//
//  Created by Jim Yang on 2023-09-12.
//
...
//            self?.headerView.followerCountLabel.text = String(user.followersCount)
//            self?.headerView.followingCountLabel.text = String(user.followingCount)
            ...
            self?.headerView.userBioLable.text = user.bio
            self?.headerView.profileAvatarImageView.sd_setImage(with: URL(string: user.avatarPath))
            self?.headerView.joinDateLable.text = "Joined \(self?.viewModel.getFormattedDate(with: user.createdOn) ?? "")"
